...
x_dist <- seq(0, 30, length = 100)

plot(x_dist, dgamma(x_dist, shape = in_shape, scale = in_scale))
```
...
step_hist[1,] <- current_pos



for(i in 2:(n_steps+1)){
  
  next_pos <- move_agent(current_pos, current_dir, var_dir, in_shape, in_scale)  
  
  current_dir <- calc_direction(current_pos, next_pos)
  
  current_pos <- next_pos
 
  step_hist[i,] <- current_pos 
  
}

plot(step_hist); lines(step_hist)

```

_NB_ at this stage, some 100,000 steps (over 2 months of simulation at 1 second resolution) take < 1 second to complete.
...
```{r}
  ...
  bound_repel <- terrain(bound_decay, v = 'aspect', unit = "radians"); 
  values(bound_repel) <- -(values(bound_repel) - 0.5*pi) # conversion from clockwise bearing from north
  plot(bound_repel)
...
start_pos <- st_sample(st_as_sfc(st_bbox(bound_repel)), 1)
...
step_hist <- st_as_sf(current_pos)
...
for(i in 2:(n_steps+1)){
  ...
  step_hist <- rbind(step_hist, current_pos)
...
start_pos <- st_sfc(st_point(c(1e5, 626e4)), crs = crs_utm31)
...
for(i in 2:(n_steps+1)){
  ...
  current_dir <- extract(bound_repel, vect(next_pos), ID = F) %>%
    as.numeric()
...
ggplot() + 
  tidyterra::geom_spatraster(data = bound_repel, alpha = 0.5) +
  geom_sf(data = step_hist, col = "purple", alpha = 0.5)
...
```{r}
set.seed(7657)
n_steps <- 1000 # one day

start_pos <- st_sfc(st_point(c(1e5, 6295000)), crs = crs_utm31)
...
for(i in 2:(n_steps+1)){
  ...
  dir_modifiers <- extract(bound_repel, vect(next_pos), ID = F) 
  
  travel_dir <- atan2(sin(current_dir) + sin(dir_modifiers$aspect)*dir_modifiers$slope,
                      cos(current_dir) + cos(dir_modifiers$aspect)*dir_modifiers$slope)
...
```{r}
ggplot() + 
  tidyterra::geom_spatraster(data = bound_repel$aspect, alpha = 0.5) +
  geom_sf(data = step_hist, col = "purple", alpha = 0.5)
...
for(i in 2:(n_steps+1)){
  ...
  dir_modifiers <- extract(coast_repel, vect(next_pos), ID = F) 
...
```{r}
ggplot() + 
  tidyterra::geom_spatraster(data = coast_repel$aspect, alpha = 0.5) +
  geom_sf(data = step_hist, col = "purple", alpha = 0.5)
