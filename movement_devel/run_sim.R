run_sim <- function(in_agent, in_species, in_imb, in_ibm_config, in_density = density_map){

  #daylight_refs <- suncalc::getSunlightTimes(lat = in_agent@history$geometry[1], lon = ref_loc[2], date = as_date(current_time))
  ...
  while(current_time <= in_ibm_config@end_date){
    ...
    if(date(new_time) != date(current_time)) {
      ...
      energy_profile <- calc_day_cost(in_agent = in_agent, in_species = in_species,
                                      in_ibm = in_ibm, sst = 8, intake = units::set_units(589.5, "kJ/h"))
