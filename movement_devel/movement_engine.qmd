...
set.seed(7657)
n_steps <- 1000 # 3 months, 10 min steps = 12960
prop_hr <- 1/6
n_agents <- 100
current_time <- now() 
tz(current_time) <- "UTC"
sim_day <- 1
energy_in_step <- rnorm(1, 22, 2)
ref_loc <- c(56.2, -2.56)
agent_wt <- 1000
dive_duration <- runif(1, 1, 5)

step_duration <- minutes(10)

# notably faster to work directly with seconds
working_time <- current_time %>% as.numeric()
working_duration <- step_duration %>% as.numeric()

pop_size <- 1e5
pop_wt <- pop_size/n_agents

SST <- 15

state_df <- list(A = c(0.1, 0.1), B = c(2, 1), C = c(4, 2))
  
trans_mat <- matrix(c(80, 5, 9, 5, 5, 
                      8, 80, 9, 5, 5,
                      4, 5, 80, 10, 5, 
                      4, 5, 1, 70, 5, 
                      4, 5, 1, 10, 80), ncol = 5)
...
# flight/colony/active/inactive/diving - per step
state_expenditure <- c(141, 33.8, 113-(2.75*SST), 72.2-(2.75*SST), -1000 + 3.71*(sum(1-exp(-dive_duration/1.23))))
                                                                                   
energy_out <- 0
max_intake_rate <- 2.95 #g per minute: seabord (p127)
energy_intake <- 6.1 # j per g: seabord (p127)
...
step_hist <- matrix(nrow = n_steps+1, ncol = 7) %>% 
  as.data.frame()

step_hist[1, ] <- c(unlist(start_pos), start_state, energy_out, current_time, sim_day, energy_intake)

names(step_hist) <- c("x", "y", "current_state", "energy_out", "sim_time", "sim_day", "energy_intake")

system.time({
  
daylight_refs <- suncalc::getSunlightTimes(lat = ref_loc[1], lon = ref_loc[2], date = as_date(current_time))
...
```{r}
ggplot(step_hist) + 
  geom_point(aes(sim_time, c_energy, col = current_state)) +
  theme_light()
