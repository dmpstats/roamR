...
trans_mat <- matrix(c(90, 5, 5, 2, 70, 10, 8, 25, 85), ncol = 3)
...
state_expenditure <- c(141, 33.8, 113-(2.75*SST))*prop_hr
...
step_hist <- matrix(nrow = n_steps+1, ncol = 4) 
step_hist[1, ] <- c(unlist(start_pos), start_state, energy_out)
...
for(i in 2:(n_steps+1)){
  ...
  step_hist[i,] <- c(unlist(next_pos), current_state, current_expenditure)
...
step_hist <- step_hist %>% 
  as.data.frame() %>% 
  sf::st_as_sf(coords = c(1, 2)) %>% 
  rename(state = V3, energy_out = V4) %>% 
  mutate(state = factor(state), cum_energy = cumsum(energy_out)) %>% 
  st_set_crs(crs_utm31)
```
