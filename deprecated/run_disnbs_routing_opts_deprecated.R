...
#'
#' Run the model developed for the DisNB simulation. This approach is based on a
#' density-informed movement model, where tracks are stochastically generated
#' via density distribution maps.
#'
#'
#' @param ibm <[IBM-class]> created via [rmr_initiate()].
#' @param run_scenario <[string][character]> which scenario(s) to run in the
...
#' @param dens_id <[string][character]> the identifier for the `<Driver>` object
...
#'   more information.
#' @param intake_id <[string][character]> the identifier for the `<Driver>`
#'   object containing the agent's baseline energy intake map.
#' @param imp_dens_id <[string][character]> the identifier for the `<Driver>`
#'   object containing the agent's impacted density map.
#' @param imp_intake_id <[string][character]> the identifier for the `<Driver>`
#'   object containing the agent's impacted energy intake map.
#' @param feed_avg_net_energy <[units]>, the average net energy per unit of
#'   feeding time. This is a tuning parameter expressing the energy required to
#'   balance the energetic equations for an average agent.
#' @param target_energy <[units]>, the target daily net energy. Constitutes
...
#'   the following day.
#' @param smooth_body_mass `<bm_smooth_opts>` object, specifying
...
run_disnbs <- function(
  ...
  if (isFALSE(quiet)) {
    cli::cli_h1("Running the DisNBS Individual-Based Model")
  }

  # input validation -------------------------
  if (isFALSE(quiet)) {
    cli::cli_progress_step(
      "Performing validation checks on inputs and underlying data."
    )
  }
  ...
  if (not_null(imp_dens_id)) {
    check_class(imp_dens_id, "character")
  }
  if (not_null(imp_intake_id)) {
    check_class(imp_intake_id, "character")
  }
  ...
  if (run_scen %in% c("baseline", "baseline-and-impact")) {
    if (is.null(intake_id)) {
      cli::cli_abort(
        "{.arg intake_id} must be provided when {.code run_scen = {.val {run_scen}}}"
      )
    }
  }
  ...
  if (run_scen %in% c("impact", "baseline-and-impact")) {
    if (is.null(imp_dens_id)) {
      cli::cli_abort(
        "{.arg imp_dens_id} must be provided when {.code run_scen = {.val {run_scen}}}"
      )
    }
    if (is.null(imp_intake_id)) {
      cli::cli_abort(
        "{.arg imp_intake_id} must be provided when {.code run_scen = {.val {run_scen}}}"
      )
    }
  }
  ...
  if (run_scen %in% c("impact", "baseline-and-impact")) {
    ...
    if (any(dns_dim != imp_dns_dim)) {
      ...
      txt_r <- paste(
        paste(names(imp_dns_dim), imp_dns_dim, sep = ':'),
        collapse = ", "
      )
  ...
  # dependent on daily night fraction, such as state_balance(), fall apart.
  ...
  if (isFALSE(quiet)) {
    cli::cli_progress_step("Preparing and configuring data for simulation.")
  }
  ...
  if (run_scen %in% c("baseline", "baseline-and-impact")) {
    if (isFALSE(quiet)) {
      cli::cli_progress_step(
        "Simulating agents' journeys under the baseline-case scenario"
      )
    }
    ...
    agents_bsln <- furrr::future_map(
      ...
      .options = furrr::furrr_options(
        seed = TRUE,
        #globals = c(names(rlang::global_env()), names(rlang::current_env())),
        packages = c(
          "stars",
          "sf",
          "purrr",
  ...
  if (run_scen %in% c("impact", "baseline-and-impact")) {
    if (isFALSE(quiet)) {
      cli::cli_progress_step(
        "Simulating agents' journeys under the impact-case scenario"
      )
    }
...
derive_night_cube <- function(
  ...
  tgrd <- seq(
    start_date,
    end_date + lubridate::period(delta_time),
    by = delta_time
  )
  ...
  npr <- array(
    rep(npr, length(lon)),
    dim = c(length(tgrd), length(lat), length(lon))
  ) |>
    aperm(c(3, 2, 1))
...
#' Internal helper to extract data and info required for configuring DisNBS's agent
#' simulation model
#'
#' This is a non-userfacing function to be used within `run_disnbs()`. Extracts and process information relevant to the simulation.
#'
#'
#'
...
#'
#' @param dens_drv <[Driver-class]> with baseline density surface.
#' @param ibm_cfg <[ModelConfig-class]> object.
#' @param ids <[list]> listing driver ID names used by [simulate_agent_disnbs()] to
#'   select the relevant driver.
#' @param waypnts_res <[units]> the waypoint resolution for route paths.
#'   Default is 100 meters.
#' @param reroute_prop <[numeric]> the proportion of the simulation timespan between
#'   re-routing events (default 0.1, i.e. agents reroute every 10th of the
#'   simulation period). Ignored when `dens_drv` has a temporal dimension, in
#'   which case its temporal resolution determines re-routing timesteps.
#' @param bmsm_option `<bm_smooth>` providing options for the smoother to apply
...
#'
#' Under "periodic" routing, re-routing timesteps are defined by two contributing factors:
#'  a) input `routing$frequency`: as the proportion of the simulation's timespan
#'  span, i.e. re-routing every (`frequency`*100)-th of the simulation period.
#'  b) the temporal resolution of the `dens_drv` datacube, if it has a temporal dimension.
...
create_dnbs_config <- function(
  ...
  # COMEBAK: will need revisiting if/when implementing asynchronous agent starting dates. Ideally via further processing inside agents' simulation function
  time_grid <- seq(
    ibm_cfg@start_date,
    ibm_cfg@end_date,
  ...
  if (routing$method == "periodic") {
    span_days <- difftime(ibm_cfg@end_date, ibm_cfg@start_date, units = "days")

    freq_routing_dates <- seq(
      ibm_cfg@start_date,
      ibm_cfg@end_date - lubridate::period(ibm_cfg@delta_time),
      by = span_days * routing$frequency
    )

    routing_timesteps <- unique(c(
      routing_timesteps,
      which(time_grid %in% freq_routing_dates)
    ))
  }
  ...
  if (not_null(dns_nrst_meta)) {
    ...
    if (length(nrst_tm_idx) > 0) {
      ...
      dns_tm_slices <- if (tm_cl %in% c("Date", "POSIXct", "POSIXlt")) {
      ...
      if (any(is.na(dns_tm_slices))) {
        cli::cli_abort(
          c(
            "Temporal dimension in datacube of driver {.val {dens_drv@id}} does not fully match all time steps under modeling.",
      ...
      # get routing timesteps from density surface
      if (routing$method == "periodic") {
        routing_timesteps <- unique(c(
          routing_timesteps,
          which(diff(na.exclude(dns_tm_slices)) != 0) + 1
        ))
      }
    }
  ...
  structure(
    append(
      list(
        ...
        routing_timesteps = sort(routing_timesteps), # timesteps (i.e. indices of simulation's timepoints) at which routing paths must be established. NULL if `routing_method` is "sequential"
        ...
        waypnts_res = routing$waypnts_res,
        bm_smooth = bm_smooth
      ),
      ids
    ),
...
#' Routing options for the DisNBS simulation model
#'
#'
#'
#' Helper function to configure how agents route their movement tracks during the
#' DisNBS simulation.
#'
#'
#' @param method method to use for routing path generation. Options are:
#'   - `"periodic"`: routes are generated at fixed timesteps of the simulation, determined by the
#'

#'
#'   - `"sequential"`: (default): routes are generated sequentially as an agent reaches the
#'     end location of the current route (i.e. when the agent reaches the end of the current
#'     route, a new route is generated). This means that the timing of re-routing events
#'     is determined by the length of the route and the travelling speeds of the agent.
#'
#'
#'     `frequency` parameter and the temporal resolution of the density surface. For example, with
#'     `frequency = 0.1`, routes would be generated every 10% of the simulation period. In addition, if the density surface has a temporal dimension, re-routing events will be scheduled at the time-steps corresponding to changes in the density surface, regardless of the `frequency` parameter. This means that agents will re-route at consistent time-steps across all simulations, based on the temporal resolution of the density surface. This method is particularly useful when density surfaces have low temporal resolution or when consistent re-routing is desired across all simulation timesteps, based on the density surface at that time. This method is more computationally efficient than the sequential method, as it reduces the number of routing events and allows for better parallelization of agent simulations.

#'     It may be particularly useful when density surfaces have low temporal resolution or when consistent re-routing is desired across all simulation
#'
#' timesteps, based on the density surface at that time. This method is more
#'
#' @param cycle_fraction the periodicity of agent re-routing, expressed as the regularity with which routes are generated during the simulation. Must be a numeric scalar between 0 and 1, where smaller values correspond to more frequent re-routing. Ignored when `method = "sequential"`, in which case the timing of re-routing events is determined by the length of the route and the travelling speeds of the agent.
#'
#' period between re-routing events, used when `method = "periodic"`. For example, with `cycle_fraction = 0.1`, routes would be generated every 10% of the simulation period. Ignored when `method = "sequential"`, in which case the timing of re-routing events is determined by the length of the route and the travelling speeds of the agent.

#' @param waypnts_res the resolution of waypoints along the generated routes, specified as a `units` object.
#' For example, `units::set_units(100, "m")` would generate waypoints every 100 meters of the route. Can be provided
#' as a numeric (e.g. `100`) in which case it will be assumed to be in meters.
#'
#' @returns
#' A list of class `<routing_opts>` containing the specified routing options, to be used in the `config` argument of `create_dnbs_config()`, and ultimately in the `config` argument of `simulate_agent_disnbs()`.
#'
#' @examples
#' routing_opts("periodic", cycle_fraction = 0.4, waypnts_res = 6)
#'
#' @export
routing_opts <- function(
  method = c("periodic", "sequential"),
  cycle_fraction = 1,
  ...
  ## `cycle_fraction` must be scalar between 0 and 1, when method is "periodic". Ignored otherwise
  if (method == "periodic") {
    if (
      !is.numeric(cycle_fraction) ||
        length(cycle_fraction) != 1L ||
        is.na(cycle_fraction) ||
        cycle_fraction <= 0 ||
        cycle_fraction > 1
    ) {
      cli::cli_abort(
        "`cycle_fraction` must be a numeric scalar with 0 < cycle_fraction <= 1."
      )
    }
  }

  ## `waypnts_res` must be a positive finite numeric scalar with length 1, and with length units.
  ## Can be provided as a numeric (e.g. `100`) in which case it will be assumed to be in meters.
  ## b
  if (length(waypnts_res) != 1L) {
    cli::cli_abort(
      "`waypnts_res` must have length of 1."
    )
  }

  if (inherits(waypnts_res, "numeric")) {
    waypnts_res_value <- waypnts_res
    waypnts_res <- units::set_units(waypnts_res, "m")
  } else {
    check_class(waypnts_res, "units")
    check_units_contextual(waypnts_res, context = "length")
    waypnts_res_value <- as.numeric(waypnts_res)
  }

  if (
    is.na(waypnts_res_value) ||
      !is.finite(waypnts_res_value) ||
      waypnts_res_value <= 0
  ) {
    cli::cli_abort(
      "`waypnts_res` must have a positive finite value."
    )
  }

  # return list with specified routing options
  structure(
    list(
      method = method,
      frequency = frequency,
