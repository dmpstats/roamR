...
simulate_agent_disnbs <- function(
  ...
  # NOTES:
  #  - derivation of agent locations on tracks: sf::st_line_interpolate() was considered for this, but it requires distances to be in same units as the linestring projection. This would require extra tranformations to UTM and back, which may be computationally expensive. Instead, the current segmentizing approach should provide a good approximation, as long as the `waypnts_res` is set to a sufficiently small value.
  ...
  for (step in seq_along(dnbs_cfg$time_grid)) {
    ...
    reroute <- if (step == 1L || dnbs_cfg$dns_routing[step]) {
      ...
      total_dist >= cum_dist[length(cum_dist)]
    }
    ...
    if (reroute) {
      ...
      if (!sf::st_is_empty(actv_track)) {
        # segmentize track to specified point distance
      ...
      # distance between track's waypoints (meters)
      seg_dists <- sf::st_distance(
        actv_waypnts[-length(actv_waypnts)],
        actv_waypnts[-1],
        by_element = TRUE
      ) |>
        assert_units_to_numeric("m")

      # cumulative track's length at each waypoint (meters)
      cum_dist <- c(0, cumsum(seg_dists))
    ...
    step_loc <- actv_waypnts[which.min(abs(total_dist - cum_dist))] |>
      sf::st_sf(tm = dnbs_cfg$time_grid[step], geometry = _)
