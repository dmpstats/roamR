...
p_bdm <- guill_history |> 
  ggplot() +
  geom_line(aes(x = Date, y = body_mass_smooth, col = scenario), size = 1) +
...
p_activity <- guill_history |> 
  ...
  theme(legend.position = "bottom") +
  labs(y = "Activity budget (day proportion)", x = "timestep", title = "Susceptible Agent")
...
p_activity <- guill_history |> 
  ...
  facet_wrap(~scenario, ncol = 1) +
  theme(legend.position = "bottom") +
  labs(y = "Activity budget (day proportion)", x = "timestep", title = "Non-susceptible Agent")
