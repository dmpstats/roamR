...
#'
#' @slot id <[`string`][character]> the unique identifier for the species.
#' @slot common_name <[`string`][character]> the common name of the species.
#' @slot scientific_name <[`string`][character]> the scientific name of the species.
#' @slot body_mass_distr an object of class [VarDist-class], defining the
#'   distribution of body mass for the species.
#' @slot energy_to_mass_distr <[`VarDist-class`]> the energy-to-bodymass conversion rate for the species (e.g. g/kJ). Determineshow gained or lost energy translates into changes in body mass.
#' @slot states_profile <[`list`]> of <[`State-class`]> objects, defining the
...
#'
#' @slot mortality_thresh_distr <[`VarDist-class`]> values of agent's condition (e.g. body mass), below which it is assumed to have died (*Note: slot may require further generalization*)
...
#'
#'
#' @param id <[`string`][character]> the unique identifier for the species.
#' @param common_name <[`string`][character]> the common name of the species.
#' @param scientific_name <[`string`][character]> the scientific name of the species.
#' @param body_mass_distr <[`VarDist-class`]> the distribution of body mass for the species.
#' @param energy_to_mass_distr <[`VarDist-class`]> the energy-to-bodymass conversion rate for the species (e.g. g/kJ). Determines how gained or lost energy translates into changes in body mass.
#' @param states_profile <[`list`] of <[`State-class`]> objects, defining the
#'   behavioural/activity states for the species to consider in the model.
...
#'
#' @param mortality_thresh_distr <[`VarDist-class`]> the values of agent's condition (e.g. body mass), below which it is assumed to have died (*Note: parameter may require further generalization*)
...
Species <- function(
  ...
  if (is(states_profile, "State")) {
    states_profile <- list(states_profile)
  }
  if (is(driver_responses, "DriverResponse")) {
    driver_responses <- list(driver_responses)
  }
  ...
  if (length(states_profile) > 0) {
    check_class(states_profile, "State", inlist = TRUE)
  }
  if (length(driver_responses) > 0) {
    check_class(driver_responses, "DriverResponse", inlist = TRUE)
  }
...
methods::setValidity("Species", function(object) {
  ...
  if (length(errors) == 0) {
    ...
    do.call(paste, list(errors, collapse = " "))
  }
})
