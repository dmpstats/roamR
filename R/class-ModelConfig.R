...
ModelConfig <- function(
  ...
  if (!is(aoc_bbx, "bbox")) {
    ...
    cli::cli_abort(
      "{.arg aoc_bbx} and {.arg ref_sys} must refer to the same CRS"
    )
  }

  if (is.numeric(n_agents)) {
  ...
  if (movement_model == "di") {
    delta_x <- delta_y <- NA_real_
  }
...
#### setter
#' @export
setGeneric("movement_model<-", function(x, value) {
  standardGeneric("movement_model<-")
})
...
setMethod("show", "ModelConfig", function(object) {
  ...
  cli::cat_line(cli::format_message(
    "{.cls {is(object)[[1]]}} instance with attributes:"
  ))
  ...
  bbox_txt <- paste0(
    ...
    paste(
      sapply(1:4, \(i) {
        paste(names(object@aoc_bbx)[i], object@aoc_bbx[i], sep = ": ")
      }),
      collapse = "  "
    ),
    " ",
  ...
  sim_duration <- difftime(
    as.Date(object@end_date),
    as.Date(object@start_date),
    units = "days"
  )
...
textify_site <- function(x, word_start, align_width, n = 5) {
  if (nrow(x) == 0) {
    ...
    paste0(
      format(
        paste0(word_start, " ", ifelse(nrow(x) == 1, "site", "sites"), ":"),
        width = align_width
      ),
