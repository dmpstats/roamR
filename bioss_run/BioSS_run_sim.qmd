...
# Spreadsheet sets to 67%, with (95% ?) CI of 53-77 - about sd of 6.1 if normal approx
...
sequent_imp_runs <- furrr::future_map(agent_list, \(x) bioss_run_sim(
            ...
            in_density = guill_ibm@drivers$dens@stars_obj,
            mean_intake = 543,
            impact_map = guill_ibm@drivers$energy_imp@stars_obj,
            in_imp_density = guill_ibm@drivers$dens_imp@stars_obj), 
            .progress = T, .options = furrr::furrr_options(seed = T))
...
```{r}
ggplot(imp_hist_obj, aes(x = timestep, y = units::drop_units(body_mass))) + 
  geom_point(fill = "grey", col = "black", alpha = 0.5) +
  xlab("day") +
  ylab("Implied body-mass (g)") +
  geom_smooth() +
...
energy_summ <- comp_df |> 
  ...
  mutate(mean_energy = units::drop_units(mean_energy),
         mean_mass = units::drop_units(mean_mass))
...
ggplot(energy_summ) +
  ...
  ggtitle("Mean energy expenditure difference", "Impacted vs unimpacted") +
  ylab("Mean energy expenditure (kJ) over season") +
  theme_light()
...
ggplot(activity_summ) +
  ...
  ylab("Mena difference impact - unimpacted (mins per day)") +
...
ggplot(test) +
geom_bar(aes(x = impact, y = proportion, fill = activity), stat = "identity") +
  theme_light()
