...
sequent_runs <- map(agent_list[1:4], \(x) {saveRDS(.Random.seed, "in_seed.rds"); bioss_run_sim(
            in_agent = x, in_ibm = guill_ibm,
            in_species = guill, 
            in_ibm_config = guill_imb_config,
            mean_intake = 543,
...
saveRDS(sequent_runs, "data/sequent_runs.rds")
...
#hist_obj <- lapply(hist_obj, \(x) x |> mutate(body_mass = ksmooth(timestep, body_mass, bandwidth = 7, kernel = "normal")$y))
...
```{r}
ggplot(hist_obj, aes(x = timestep, y = body_mass)) + 
...
saveRDS(sequent_imp_runs, "data/sequent_imp_runs.rds")
...
#imp_hist_obj <- lapply(imp_hist_obj, \(x) x |> mutate(body_mass = ksmooth(timestep, body_mass, bandwidth = 7, kernel = "normal")$y))
...
imp_ind <- lapply(sequent_runs, \(x) x@properties@move_influences$owf_imp) %>% unlist()
impact_table <- data.frame(agent_id = paste0("agent_", 1:length(sequent_runs)), owf_sensitive = imp_ind)
...
wt_summ <- comp_df |> 
  group_by(agent_id, impact) |>
  summarise(
    end_wt = last(body_mass)
  ) 


ggplot(wt_summ) +
  geom_point(aes(agent_id, end_wt, col = impact), size = 3) +
  ggthemes::scale_color_fivethirtyeight() +
  ggtitle("NBS final weight", "Impacted vs unimpacted") +
...
wt_summ <- comp_df |> 
  ...
  group_by(agent_id) %>% 
  summarise(diff = end_wt[2] - end_wt[1]) %>% 
  left_join(impact_table) %>% 
  mutate(diff = diff, owf_sensitive = factor(owf_sensitive))

wt_summ %>% 
  ggplot() +
  geom_histogram(aes(diff, colour = owf_sensitive, fill = owf_sensitive)) +
  facet_wrap(~owf_sensitive, ncol = 1)

wt_summ %>% 
  ggplot() +
  geom_histogram(aes(diff, colour = owf_sensitive, fill = owf_sensitive), alpha = 0.4)


wt_summ %>% 
  group_by(owf_sensitive) %>% 
  summarise(mean = mean(diff), med = median(diff), sd = sd(diff))
