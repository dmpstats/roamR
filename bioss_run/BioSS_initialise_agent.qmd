...
A BioSS branch of the `roamR` package can be found here (). The package is in development - we will not install this branch `per se`, rather access directly i.e. open the package project () and use `devtools` to access the namespace.  
...
The objective here is to create an individual agent, in preparation for interacting with its environment. `roamR` 
